...
export const EncryptedText = ({
  ...
  return (
    <motion.span ref={ref} className={`${cn(className)} text-[${fontSize}]`} aria-label={text} role="text"
    ...
    >
      {text.split("").map((char, index) => {
        ...
        return (
          <span
            key={index}
            className={`${cn(isRevealed ? revealedClassName : encryptedClassName)} !bg-gradient-to-br !from-blue-400 !to-red-500  !bg-clip-text !text-transparent`}>
            {displayChar}
          </span>
        );
      })}
